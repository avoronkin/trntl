...
module.exports = class Client extends net.Socket{
    ...
    async ping () {
        ...
        const packet = makePacket(REQUEST.PING, id)
    ...
    async auth () {
        ...
        const body = Buffer.allocUnsafe(user.byteLength + chapSha1.byteLength + scramble.length + 15)

        body[0] = 0x82
        body.writeUInt8(IPROTO.USER_NAME, 1)
        user.copy(body, 2)
        body[2 + user.byteLength] = IPROTO.TUPLE
        body[3 + user.byteLength] = 0x92
        chapSha1.copy(body, 4 + user.byteLength)
        body[4 + user.byteLength + chapSha1.byteLength] = 0xb4
        scramble.copy(body, 5 + user.byteLength + chapSha1.byteLength)

        const packet = makePacket(REQUEST.AUTH, id, body)
